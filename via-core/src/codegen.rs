...
pub fn generate(resources: &[Resource]) -> Result<GenerationOutput> {
    ...
    for resource in resources {
        ...
        if let Some(model) = &resource.model {
            ...
            let contents = render_model(resource, model);
        ...
        let contents = render_controller(resource, &controller, resource.model.as_ref());
        output.push(path, contents);
    }
...
fn render_model(resource: &Resource, model: &Model) -> String {
    ...
    let controller = resource
        ...
        .clone()
        .unwrap_or_else(default_controller);
    let param_structs = build_param_structs(struct_name, &controller, Some(model));
